...
def verificaDataAnuncio(dataReferencia):
    ...
    dataAnuncio=datetime.strptime(dataVencimento, '%Y-%m-%d')+timedelta(days=1)
...
def anuncios(request):
    ...
    anuncios = Anuncio.objects.order_by('-created_at').filter(local_municipio__icontains=busca).filter(status_excluir=False).filter(Q (anuncio_fim__date__range=(datetime.now(), datetime.now()+timedelta(days=15))))
...
def anuncioDetalhes(request, slug2):
    ...
    if request.user.is_authenticated:
        conversaReferencia = Conversa.objects.order_by('created_at').filter(anuncio_id=anuncio.id).filter(motorista_id=request.user.email).filter(proprietario_id=anuncio.email).first()
        msgs = Mensagem.objects.order_by('-created_at').filter(anuncio_id=anuncio.id).filter(motorista_id=request.user.email).filter(proprietario_id=anuncio.email)    
        total=len(msgs)
        print(type(total))
    if request.method=='POST':
        ...
        if form.is_valid():            
            ...
            def enviar_email(): #envia a mensagem par o email do proprietário
                ...
                html_content = '<strong><span style="color: #BDBDBD;">ALUGUE</span><span class="at" style="color: #FBC02D;">TÁXI</span></strong><br><br>' + mensagem_p1 + '"'+ request.POST['mensagem'] + '"<br><br>De: ' + request.user.first_name + '<br><br>http://localhost:8000/anuncio/mensagens' + mensagem_p2 
                msg = EmailMultiAlternatives(assunto, text_content, 'contato@aluguetaxi.com.br', [email])
                msg.attach_alternative(html_content, "text/html")
                msg.send()

                messages.add_message (request, SUCESSO, 'Sua mensagem foi enviada com sucesso! Aguarde resposta do propietário do veículo!')
                messages.add_message (request, SUCESSO, 'As perguntas também são enviadas para o e-mail do proprietário.')
                messages.add_message (request, SUCESSO, 'A conversa é privada - não pode ser visualizada por outros usuários.')
            ...
            if conversaReferencia: #Verifica se já existe a conversa
                ...
                Conversa.objects.filter(id=conversaReferencia.id).update(updated_at=datetime.now().astimezone(fuso_horario))
                ...
                conversa.motorista_nome=str.replace(str(request.user.first_name)," ","")
                conversa.motorista_sobrenome=str.replace(str(request.user.last_name)," ","")
                conversa.proprietario_nome=str.replace(str(anuncio.nome)," ","")
                conversa.proprietario_sobrenome=str.replace(str(anuncio.sobrenome)," ","")
                conversa.save()
...
def anuncioLista(request):    
    anuncios = Anuncio.objects.order_by('-created_at').filter(id_usr=request.user)
...
def anuncioNovo(request):    
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            messages.info (request,  'Anúncio cadastrado com sucesso: ' + anuncio.modelo )
...
def anuncioInfo(request, slug2):
    ...
    msgs = Mensagem.objects.order_by('created_at').filter(anuncio_id=anuncio.id).filter(proprietario_id=anuncio.email)    
    ...
    if anuncio.id_usr == request.user:
        ...
        if request.method=='POST':
            ...
            if form.is_valid():
                ...
                Conversa.objects.filter(id=request.POST['conversa_id']).update(updated_at=datetime.now().astimezone(fuso_horario))

                messages.add_message (request, SUCESSO, 'Sua mensagem foi enviada com sucesso! Aguarde resposta do usuário!')
                messages.add_message (request, SUCESSO, 'A conversa é privada - não pode ser visualizada por outros usuários.')
                messages.add_message (request, SUCESSO, 'As perguntas e respostas também são enviadas para os respectivos e-mails.')
...
def anuncioEditar(request, slug2):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            anuncio.local_municipio=request.user.cidade
            anuncio.save()
            messages.info (request,  'Anúncio atualizado com sucesso: ' + anuncio.marca + ' ' + anuncio.modelo)
            return redirect('/anuncio/info/' + anuncio.slug2)
        else:
...
def anuncioMensagens(request):
    conversas = Conversa.objects.annotate(search=SearchVector('proprietario_id', 'motorista_id')).filter(search=request.user.email).order_by('-updated_at')
    msgs = Mensagem.objects.annotate(search=SearchVector('proprietario_id', 'motorista_id')).filter(search=request.user.email).order_by('created_at')
    total = len(conversas)
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            mensagem_p1 = 'Existe uma nova mensagem no AlugueTáxi para você:<br><br>'
            mensagem_p2 = '<br><br>Observação: é preciso estar logado no site da AlugueTáxi para poder acessar a seção de mensagens!<br><br>Equipe AlugueTáxi'
            assunto = 'AlugueTáxi | Mensagem | ' + mensagem.nome + ' | ' +  request.user.first_name
            text_content = request.POST['mensagem']
            html_content = '<strong><span style="color: #BDBDBD;">ALUGUE</span><span class="at" style="color: #FBC02D;">TÁXI</span></strong><br><br>' + mensagem_p1 + '"'+ request.POST['mensagem'] + '"<br><br>De: ' + request.user.first_name + '<br><br>http://localhost:8000/anuncio/mensagens' + mensagem_p2 
            msg = EmailMultiAlternatives(assunto, text_content, 'contato@aluguetaxi.com.br', [email])
            msg.attach_alternative(html_content, "text/html")
            ...
            messages.add_message (request, SUCESSO, 'Sua mensagem foi enviada com sucesso! Aguarde resposta do usuário!')
            messages.add_message (request, SUCESSO, 'As perguntas também são enviadas para o e-mail do usuário.')
...
def anuncioExluir(request, slug2):
    ...
    messages.info (request,  'Este anúncio foi deletado: ' + anuncio.marca + ' ' + anuncio.modelo)
...
def fotoExluir(request, slug2, id):
    ...
    if foto.id_usuario == str(request.user.id):   
        foto.delete()
        messages.info (request,  'Foto excluída para este anúncio: ' + anuncio.marca + ' ' + anuncio.modelo)
    else:
...
def uploadFotos(request, slug2):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            anuncio_foto.save()
            messages.info (request, 'Foto cadastrado com sucesso para o anúncio: ' + anuncio.marca + ' ' + anuncio.modelo)  
...
def anuncioAtivar(request, slug2):
    ...
    anuncio.anuncio_fim=datetime.now().astimezone(fuso_horario)+timedelta(days=15)
    anuncio.save()

    data_inicio=retornaData(anuncio.anuncio_inicio)
    data_fim=retornaData(anuncio.anuncio_fim)
    
    mensagem_p1 = 'Olá, a sua promoção mensal de anúncio foi ativada <strong>(7 dias grátis)!</strong><br><br>' + '- Período de exibição do anúncio: de ' + data_inicio +' - '+ data_fim + '<br><br>- Seus anúncios podem ser renovados para um novo período de 15 dias, sempre que você precisar. Basta ir na sessão Meus Anúncios do nosso site e reativar qualquer anúncio cadastrado anteriormente, mediante pagamento de uma nova taxa de R$ 15,00 (valor atual) por anúncio.'
    mensagem_p2 = '<br><br>- As informações cadastradas para o seu anúncio podem ser editadas a qualquer tempo, sempre que você achar necessário, basta ir na sessão Meus Anúncios do nosso site.<br><br>- Link para o seu anúncio: http://localhost:8000/anuncio/detalhes/' + slug2 + '<br><br>Agradecemos por utilizar nossos serviços,<br><br>Equipe AlugueTáxi.'
    ...
    messages.info (request,  anuncio.marca + ' ' + anuncio.modelo + ' | Período do Anúncio: '+ data_inicio +' - '+ data_fim)
